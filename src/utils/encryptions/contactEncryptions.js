...
import { CONTACT_ENCRYPTION_KEY } from "#config/config";
...
export const encryptContact = (data) => {
    const iv = crypto.randomBytes(IV_LENGTH);
    const cipher = crypto.createCipheriv(ALGORITHM, CONTACT_ENCRYPTION_KEY, iv);
...
export const decryptContact = (encrypted) => {
    let iv = Buffer.from(encrypted.iv, "hex");
    let encryptedText = encrypted.data;
    let decipher = crypto.createDecipheriv(ALGORITHM, CONTACT_ENCRYPTION_KEY, iv);
