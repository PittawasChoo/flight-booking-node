...
import { PAYMENT_ENCRYPTION_KEY } from "#config/config";
...
export const encryptPayment = (data) => {
    const iv = crypto.randomBytes(IV_LENGTH);
    const cipher = crypto.createCipheriv(ALGORITHM, PAYMENT_ENCRYPTION_KEY, iv);
...
export const decryptPayment = (encrypted) => {
    let iv = Buffer.from(encrypted.iv, "hex");
    let encryptedText = encrypted.data;
    let decipher = crypto.createDecipheriv(ALGORITHM, PAYMENT_ENCRYPTION_KEY, iv);
